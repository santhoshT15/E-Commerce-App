...
const Orders = ({token}) => {
  ...
  const fetchOrders = async() =>{
    ...
    try {
      const response = await axios.post(backUrl +'/api/order/list',{},{headers: {token}})
  ...
  const statusHandler = async(e, orderId) =>{
    try {
      const response = await axios.post(backUrl +'/api/order/status', {orderId, status:e.target.value}, {headers: {token}});
      if(response.data.success){
        await fetchOrders()
      }
    } catch (error) {
